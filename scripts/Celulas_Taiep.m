...
cd ..; cd cell_images;
Im1 = im2double(imread('Taiep2_6m_PLV_BrdU488_GFAP555_SOX2647_25X (1)(b).tif'));
cd ..; cd scripts;
...
area_condicion = 300;
Condiciones = [stats1.Area] < area_condicion/5 | [stats1.Area] > area_condicion*3 | [stats1.Circularity] < 0.3;
Celulas_Depuradas = Eliminar_Nucleos(Im1_C_Seg, Condiciones);
figure();imshow(Celulas_Depuradas); title('Celulas buenas','Fontsize',15);

%Celulas que se encuentran pegadas %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Celulas_pegadas = Im1_C_Seg - Celulas_Depuradas;
Celulas_pegadas = Celulas_pegadas.*Im1_C;
figure();imshow(Celulas_pegadas);title("Celulas pegadas");

Celulas_pegadas_u = Celulas_pegadas > 40/255;
B1 = strel('disk', 5);  
Celulas2_Seg = imopen(Celulas_pegadas_u, B1); 
Celulas2_Seg = imfill(Celulas2_Seg,'holes');
figure();imshow(Celulas2_Seg);title('1. Segmentacion de las celulas','Fontsize',15);
...
figure(); imshow(Segmentacion_Celulas); title('Segmentacion de celulas','Fontsize',15);
...
[L5, n5, stats5, Tabla5] = Calcular_Propiedades(Segmentacion_Celulas+Celulas2_Seg);
Num5 = [1:n5]';
area5 = stats5.Area;
...
figure();imshow(Segmentacion_Celulas+Celulas2_Seg); title('Celulas enumeradas','Fontsize',15);
hold on;
for k=1:n5
    [r,c] = find(L5==k);
    rbar = mean(r);
